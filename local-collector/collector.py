...
def _normalize_spaces(txt):
    ...
    # разрядов (\xa0, U+2009, U+202F и т.п.), и набор может отличаться от
...
# Свои ID бизнеса и кампании возьмите из адресной строки своего партнёрского
# кабинета Яндекс Маркета. В публичном репозитории — только плейсхолдеры.
...
def ozon_prices_for_offer(text, offer_id):
    ...
    #     поверх цены продавца) — кабинет показывал 1916₽, а на самой
        ...
        with sync_playwright() as pw:
            ...
            for p in products:
                if p['wb_nm']:
                    ...
                    if wb_authorized is None and wb_body:
                ...
                if p['ozon_url']:
                    ...
                    # свои промо поверх цены продавца) — TEDY PINK: кабинет 1916₽,
                    # а на самой карточке реально 2183₽.
                    ozon_ok = False
                    for _ in range(3):
                        try:
