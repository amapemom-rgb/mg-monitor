...
function buildInstructionSheet() {
  ...
  var rows = [
    ...
    ['B', '12. КАК НЕ ПОЛУЧИТЬ НЕВЕРНЫЕ ДАННЫЕ'],
    ['T', 'Самое опасное в системе — не пустая ячейка, а правдоподобное неверное число. Пустоту видно сразу, подделку — нет.'],
    ['T', ''],
    ['T', 'ПУСТАЯ ЯЧЕЙКА — ЭТО СООБЩЕНИЕ, А НЕ ПОЛОМКА ТАБЛИЦЫ'],
    ['T', 'Если цену собрать не удалось, ячейка очищается. Так задумано: раньше в ней оставалось вчерашнее значение, выглядевшее свежим, и поломка парсера могла жить неделями незамеченной.'],
    ['T', 'Не заполняйте такие ячейки руками. Вписанное значение не попадёт в «Историю» и в графики, а следующий сбор его затрёт. Если всё же вписали — запустите меню «▶️ Собрать цены сейчас», иначе снимок за день останется старым.'],
    ['T', ''],
    ['T', 'ССЫЛКА — ГЛАВНЫЙ ИСТОЧНИК ВРАНЬЯ'],
    ['T', 'Ссылка ведёт не на тот товар или на другой объём — цена соберётся, будет правдоподобной и совершенно чужой. Никакая автоматика этого не поймает.'],
    ['T', 'Добавили ссылку — откройте её и убедитесь, что это именно тот товар. Проверяйте заново, если площадка переехала на новый адрес карточки.'],
    ['T', 'Одинаковая цена сразу у нескольких разных товаров — повод перепроверить ссылки. Иногда это правда (одна цена на линейку), но чаще признак ошибки.'],
    ['T', 'Пустая ссылка означает «товара там нет»: площадка молча пропускается и в отчёт о несобранном не попадает. Если товар на площадке есть, а ссылки нет, вы просто не узнаете, что цена не собирается.'],
    ['T', ''],
    ['T', 'НАЗВАНИЕ ТОВАРА НЕ ПЕРЕИМЕНОВЫВАТЬ'],
    ['T', 'Название — ключ, по которому сходятся артикулы кабинетов, история и графики. После переименования история по товару начинается с нуля, а кабинетные цены перестают находиться.'],
    ['T', ''],
    ['T', 'СЕССИИ В БРАУЗЕРЕ НА МАКЕ'],
    ['T', 'Локальный сборщик ходит по площадкам через постоянный профиль Chrome с живыми логинами. Профиль лежит в папке сборщика. В /tmp его переносить нельзя: macOS очищает эту папку при перезагрузке, и логины слетают.'],
    ['T', 'Признаки слетевшей сессии: цены WB примерно на 1% выше реальных (без Кошелька), в логе «сессия не авторизована», карточка Ozon отдаёт заглушку 18+. Лечение — один раз войти вручную: bash open_profile.sh'],
    ['T', ''],
    ['T', 'КАБИНЕТ И ВИТРИНА — РАЗНЫЕ ЧИСЛА'],
    ['T', 'Цена в кабинете продавца может отставать от реальной цены на карточке: маркетплейс накладывает свои промо поверх вашей цены. Для того, что платит покупатель, доверяйте только публичной карточке.'],
    ['T', 'Не выключайте площадку галочкой просто «чтобы не мешала». Разбег считается по включённым площадкам, и без одной из них он будет неполным, но выглядеть будет нормально.'],
    ['T', ''],
    ['T', 'ЧТОБЫ ДАННЫЕ ВООБЩЕ ПРИХОДИЛИ'],
    ['T', 'Мак должен быть включён и с работающим интернетом в час сбора. Если сети нет, запуск падает на первом же шаге и день выпадает из истории целиком.'],
    ['T', 'Каждый прогон присылает в группу короткую сводку: сколько цен собрано из скольких возможных и что не собралось. Нет строки «Не собрано» — значит всё в порядке.'],
    ['T', 'Отдельный сторож раз в сутки проверяет свежесть: если локальный сборщик молчит 2 дня или по какой-то площадке 3 дня нет новой цены, придёт сообщение. Пока причина не устранена, оно не повторяется.'],
    ['T', ''],
    ['T', 'ЕСЛИ ЧТО-ТО ПОШЛО НЕ ТАК — КУДА СМОТРЕТЬ, ПО ПОРЯДКУ'],
    ['T', '   1) Лист «Диагностика» — по строке на каждый запуск: когда, сколько минут, собрано из скольких, что не собралось, итог.'],
    ['T', '   2) Лист «Лог ошибок» — сбои источников с причиной.'],
    ['T', '   3) На Маке папка debug — при каждой неудаче туда сохраняется текст страницы и её скриншот на момент сбоя. По ним видно, была там заглушка, капча или редизайн.'],
    ['T', '   4) На Маке файл wrapper.log — полный вывод последнего запуска по каждому товару.'],
    ['T', ''],
    ['T', 'Проверить одну площадку по одному товару, не гоняя полный шестиминутный сбор и ничего не записывая в таблицу:'],
    ['T', '   cd ~/mg-monitor-local && parser_venv/bin/python3 collector.py --product LILU --source WB'],
    ['T', '   Площадки: WB, BRAND, GA, LETU, YM, OZON, OZON_CAB, YM_CAB. Товары и площадки можно перечислять через запятую.']
  ];
